...
const reportOnViolation = (context, node, {
  ...
  if (whitelistedSnapshots && node.type === _experimentalUtils.AST_NODE_TYPES.ExpressionStatement && 'left' in node.expression && (0, _tsUtils.isExpectMember)(node.expression.left)) {
    const fileName = context.getFilename();
    const whitelistedSnapshotsInFile = whitelistedSnapshots[fileName];

    if (whitelistedSnapshotsInFile) {
      const snapshotName = (0, _tsUtils.getAccessorValue)(node.expression.left.property);
...
var _default = (0, _tsUtils.createRule)({
  ...
  create(context, [options]) {
    if (context.getFilename().endsWith('.snap')) {
      ...
      return {
        CallExpression(node) {
          if ('property' in node.callee && ((0, _tsUtils.isSupportedAccessor)(node.callee.property, 'toMatchInlineSnapshot') || (0, _tsUtils.isSupportedAccessor)(node.callee.property, 'toThrowErrorMatchingInlineSnapshot'))) {
            reportOnViolation(context, node, options);
          }
        }
