...
const isThenOrCatchCall = node => node.type === _experimentalUtils.AST_NODE_TYPES.CallExpression && node.callee.type === _experimentalUtils.AST_NODE_TYPES.MemberExpression && (0, _tsUtils.isSupportedAccessor)(node.callee.property) && ['then', 'catch'].includes((0, _tsUtils.getAccessorValue)(node.callee.property));
...
const isFullExpectCall = expression => expression.type === _experimentalUtils.AST_NODE_TYPES.CallExpression && (0, _tsUtils.isExpectMember)(expression.callee);
...
const isPromiseReturnedLater = (node, testFunctionBody) => {
  ...
  if (node.type === _experimentalUtils.AST_NODE_TYPES.ExpressionStatement && node.expression.type === _experimentalUtils.AST_NODE_TYPES.CallExpression && node.expression.callee.type === _experimentalUtils.AST_NODE_TYPES.MemberExpression && (0, _tsUtils.isSupportedAccessor)(node.expression.callee.object)) {
    promiseName = (0, _tsUtils.getAccessorValue)(node.expression.callee.object);
...
const isTestFunc = node => node.type === _experimentalUtils.AST_NODE_TYPES.CallExpression && (0, _tsUtils.isSupportedAccessor)(node.callee) && [_tsUtils.TestCaseName.it, _tsUtils.TestCaseName.test].includes((0, _tsUtils.getAccessorValue)(node.callee));

const findTestFunction = node => {
  while (node) {
    if ((0, _tsUtils.isFunction)(node) && node.parent && isTestFunc(node.parent)) {
      return node;
    }
...
const verifyExpectWithReturn = (promiseCallbacks, node, context, testFunctionBody) => {
  promiseCallbacks.some(promiseCallback => {
    if (promiseCallback && (0, _tsUtils.isFunction)(promiseCallback) && promiseCallback.body) {
