...
const isDescribeEach = node => node.callee.type === _experimentalUtils.AST_NODE_TYPES.MemberExpression && (0, _tsUtils.isSupportedAccessor)(node.callee.property, 'each');
...
var _default = (0, _tsUtils.createRule)({
  ...
  create(context) {
    return {
      CallExpression(node) {
        if (!(0, _tsUtils.isDescribe)(node) || isDescribeEach(node)) {
          return;
        }
        ...
        if (!(0, _tsUtils.isStringNode)(name)) {
          context.report({
        ...
        if (!(0, _tsUtils.isFunction)(callback)) {
          context.report({
