...
const getPromiseCallExpressionNode = node => {
  ...
  if (node.type === _experimentalUtils.AST_NODE_TYPES.CallExpression && node.callee && node.callee.type === _experimentalUtils.AST_NODE_TYPES.MemberExpression && (0, _tsUtils.isSupportedAccessor)(node.callee.object) && (0, _tsUtils.getAccessorValue)(node.callee.object) === 'Promise' && node.parent) {
    return node;
  }
...
const getParentIfThenified = node => {
  ...
  if (grandParentNode && grandParentNode.type === _experimentalUtils.AST_NODE_TYPES.CallExpression && grandParentNode.callee && (0, _tsUtils.isExpectMember)(grandParentNode.callee) && ['then', 'catch'].includes((0, _tsUtils.getAccessorValue)(grandParentNode.callee.property)) && grandParentNode.parent) {
  ...
  create(context, [{
    ...
    return {
      CallExpression(node) {
        ...
        const _parseExpectCall = (0, _tsUtils.parseExpectCall)(node),
        ...
        if (expect.arguments.length > 1) {
          ...
          const expectLength = (0, _tsUtils.getAccessorValue)(expect.callee).length;
          context.report({
            loc: {
              end: {
        ...
        if (matcher.node.parent && (0, _tsUtils.isExpectMember)(matcher.node.parent)) {
          context.report({
        ...
        if (!modifier || !parentNode || !parentNode.parent || modifier.name === _tsUtils.ModifierName.not) {
          return;
        }
        /**
      ...
      'CallExpression:exit'(node) {
        if ((0, _tsUtils.isExpectCall)(node) && node.parent.type === _experimentalUtils.AST_NODE_TYPES.ExpressionStatement) {
          context.report({
